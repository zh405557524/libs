...
public final class Bar2Utils {
    ...
    public static void setStatusBarColor(@NonNull final View fakeStatusBar,
                                         @ColorInt final int color) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        Activity activity = getActivityByContextInner(fakeStatusBar.getContext());
    ...
    public static int getActionBarHeight() {
        ...
        if (Global.getContext().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, Global.getContext().getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = Global.getContext().getSystemService("statusbar");
    ...
    public static boolean isNavBarVisible(@NonNull final Window window) {
        ...
        if (isVisible) {
            ...
            if (isSamsung()
                ...
                try {
                    return Settings.Global.getInt(Global.getContext().getContentResolver(), "navigationbar_hide_bar_enabled") == 0;
                } catch (Exception ignore) {
                }
            }
    ...
    private static String getResNameById(int id) {
        try {
            return Global.getContext().getResources().getResourceEntryName(id);
        } catch (Exception ignore) {
            return "";
        }
    }
    ...
    public static boolean isSupportNavBar() {
        ...
        boolean menu = ViewConfiguration.get(Global.getContext()).hasPermanentMenuKey();
