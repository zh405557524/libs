...
public class DrakeetRetrofit {
    ...
    public DrakeetRetrofit(String BaseUrl, Interceptor interceptor) {
        ...
        try {
            open = UIUtils.getContext().getAssets().open("srca.cer");
        } catch (IOException e) {
            e.printStackTrace();
        }

//        InputStream open = new Buffer().writeUtf8(CER_12306).inputStream();
        sslParams = HttpsUtils.getSslSocketFactory(new InputStream[]{open}, null, null);
